...
| `scripts/tooling-check.sh` | Tooling: generates a key with `openssl`, then `s3armor check --bucket` clean against real MinIO, then all three `s3armor bench` tiers (local, `--backend`, `--proxy`), then confirms `--write-config`'s output is valid env `s3armor serve` accepts. |
